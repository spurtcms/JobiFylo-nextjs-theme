...
export default function ViewJobServer({ ListData, params, viewJobApi }) {
  const [relatedList, setRelatedList] = useState([]);
  const Category = useSelector((s) => s?.customerRedux?.Category_Slug_Data);
  const [viewJob, setViewJob] = useState({});
  const DetailData = useSelector(
    (s) => s?.customerRedux?.Entry_Detail_api_Data_redux
  );
  ...
  useEffect(() => {
    const relatedJobs = async () => {
      ...
      const RelatedPageApi = await fetchGraphQl(
        GET_JOB_LIST_QUERY,
        relatedValues
      );
  ...
  return (
    <>
      <main className="min-h-screen max-w-screen-2xl m-auto md:py-8 lg:px-[120px] md:px-10 p-6">
        <Link
          href="/"
          className="flex gap-1 items-center text-gray-500 text-xs font-light leading-4"
        >
          {" "}
          <img src="/img/left-arrow.svg" /> Back{" "}
        </Link>
        {DetailData ? (
          <>
            <div className="mt-8">
              <span className="px-2.5 py-1 rounded-3xl bg-blue-100 text-black text-xs font-normal">
                {DetailData?.categories?.categoryName}
              </span>
              <h2 className="mt-2 mb-4 sm:text-4xl sm:leading-[45px] font-normal text-blue-600 text-3xl">
                {DetailData?.title}
              </h2>
              ...
              <div className="flex gap-4 items-center pb-6 border-b border-gray mb-6">
                <Link
                  href={`${form_Base_url}${DetailData?.ctaLink}?id=${DetailData?.id}`}
                  target="_blank"
                  className="w-auto p-4 h-11 bg-blue-600 text-white text-base font-normal rounded flex justify-center items-center"
                >
                  Apply Now
                </Link>
              ...
              <div className="pb-6 border-b border-gray mb-6">
                <h2 className="text-2xl font-medium leading-[30px] mb-4 text-black">
                  Job Description{" "}
                </h2>
            ...
            <div>
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-medium leading-[30px]  text-black">
                  Related Jobs
                </h2>
                <Link
                  href="/"
                  className="flex items-center gap-2 text-xs font-light text-blue-600"
                >
                  {" "}
                  View All <img src="/img/right-arrow.svg" />{" "}
                </Link>
              </div>
