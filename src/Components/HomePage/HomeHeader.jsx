...
export default function HomeHeader({ setCardData, limit, offset }) {
  ...
  const handleSearchList = async () => {
    ...
    if (jobTitle !== "" || location !== "") {
      ...
      dispatch(
        search_Keyword_List(res?.ChannelEntriesList?.channelEntriesList)
      );
      // setJobTitle("");
      // setLocation("");
      ...
      <div className="bg-blue-50 w-full ">
        <div className="max-w-screen-2xl m-auto flex  gap-[130.65px] flex-col lg:flex-row lg:pt-[85px] items-start lg:ps-[120px] lg:pe-[234.5px] lg:pb-[26.89px] md:px-10 md:py-20 px-6 py-10 relative">
          <div className="lg:pt-[69px] pt-0">
            <h2 className="mb-1.5 sm:text-[64px] text-4xl font-semibold text-black-100 sm:leading-[80px] leading-10">
              Find your dreamcareer
            </h2>
            <p className="text-gray-500 text-sm leading-4 font-normal">
              Match with Companies that Share Your Values
            </p>
          </div>
            ...
            <div className="w-full bg-white h-full flex gap-px items-center p-2 rounded-md shadow-500 border border-gray mb-6 flex-col sm:flex-row">
              <div className="w-full relative flex">
                ...
                <div>
                  <img
                    src="/img/search.svg"
                    className="absolute top-3 sm:top-[27px] left-6"
                  />
                  {jobTitle !== "" ? (
                    <img
                      src="/img/cancel.svg"
                      className=" absolute top-3 sm:top-[27px] right-6 cursor-pointer "
                      onClick={() => handleClose()}
                    />
                  ) : (
                    <></>
